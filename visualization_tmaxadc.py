...
def formgrid(subj_id, data_dir, tmax_path, adc_path, T1mask_path, gt_path, flair_path, name_tag='', cluster = 125,low_lim = 10, up_lim = 60,tmax_mode=True):
    ...
    result_volume_summed = np.sum(np.sum(result_volume > 0.5) * 0.008)
    gt_volume_summed = np.sum(np.sum(gt_volume > 0.5) * 0.008)

    print('subject:', subj_id, 'result_volume', 'ground_truth')
    print(result_volume_summed, gt_volume_summed)
...
name_tag = '_adc'
data_dir = '/Users/admin/stroke_DL/results/adc/'

for subj_id  in list_nonreper_test+list_reper_test:
    # result_path = data_dir + 'prediction_' + model_name + '_fold3_' + subj_id + '.nii'
    ...
    formgrid(subj_id,data_dir,tmax_path,adc_path,T1mask_path,gt_path,flair_path,name_tag = name_tag, low_lim=23, up_lim = 73, cluster = 10, tmax_mode=False)
