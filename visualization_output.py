...
def form_grid(subj_id, data_dir, result_path, gt_path, flair_path, name_tag='',thres = 0.5, thres_gt =0.9,low_lim=10,up_lim=60,flip=False):
	...
	for s in range(0, slices):
		...
		img_output_thres = 1.0 * (result_volume[:, :, s] > thres)
...
for cv in range(0,5):
	for subj_id in list_nonreper_test[cv]:
		...
		gt_volume = gt.get_fdata()
		rows, cols, slices = gt_volume.shape
		form_grid(subj_id,data_dir,result_path,gt_path,flair_path,name_tag = name_tag, thres=thres,thres_gt=thres_gt,low_lim=20,up_lim=60, flip =False)
